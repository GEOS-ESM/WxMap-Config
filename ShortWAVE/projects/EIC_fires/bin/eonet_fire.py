...
target_url = 'https://eonet.gsfc.nasa.gov/api/v3/events/geojson?source=IRWIN&start=$sdate&end=$edate&status=all'
...
for i,p in enumerate(centroid):
    ...
    record['farea'] = float(area.iloc[i])
    record['frp'] = 0
    record['duration'] = 0
