...
class Service(MapService):
    ...
    def aggregate(self, plots):
      ...
      # Draw text on image.
        ...
        d = HersheyDraw(bg, self.config['bfont'], 36, color)

        s = self.get_label('title')
        w, h1 = d.text_size(s)
        d.draw_text( (W-w)/2, bbox[1]-h1-5, s)

        d = HersheyDraw(bg, self.config['bfont'], 16, color)

        s = self.get_label('header')
        w, h2 = d.text_size(s)
        d.draw_text( (W-w)/2, bbox[1]-h1-h2-5, s)

        d = HersheyDraw(bg, self.config['bfont'], 24, color)

        s = self.get_label('xlabel')
        w, h = d.text_size(s)
        d.draw_text( (W-w)/2, bbox[3]+5, s)
...
#           print '===> ', background
    ...
    def save_colorbar(self):
        ...
        cb = pl.colorbar(orientation="horizontal", extend="both",
                         extendrect=True, extendfrac='auto', cax=cax)
...
#       gs = [int(s) for s in gs]
        ...
        xsize = g[0]
        if gs[0] != g[0]: xsize = int((float(g[0]) / 1024.0) * float(gs[0]))
...
#                   ya     = y + (vsize * vadj/refspd)
                    ...
                    xb     = x
                    xa     = x + (vsize * uadj/refval)
                    yb     = y
                    ya     = y + (vsize * vadj/refval)
...
#       Save the background image with logos.
    ...
    def draw_symbol(self, to_img, symbols):
        """
        Draws symbols on a background image.

        This method draws symbols at specified locations on the
        background image. The symbols are specified as PNG files
        stored in the "logo_path" directory of the installation.
        ...
        symbols : list
            List of symbols. Each symbol is a list of positional arguments.
      ...
      # Paste symbols onto the background image.
        ...
        for s in symbols:
            ...
            xp = int(x /11.0 * bg.size[0])
            yp = int(y / 8.5 * bg.size[1])
            yp = bg.size[1] - yp

          # Derive name of symbol image.

            path     = self.config.get('logo_path','./')
            pathname = os.path.join(path, name) + '.png'
            if os.path.isfile(name+'.png'): pathname = name

            if not os.path.isfile(pathname): continue
          
          # Re-size the image and maintain the aspect ratio.
          # Size is specified as a percentage of the background image size

            img     = Image.open(pathname).convert("RGBA")
            ratio   = float(img.size[1]) / float(img.size[0])
          ...
          # Paste the symbol onto the background image.
            ...
            xadj = int(round(img.size[0]/2.0))
            yadj = int(round(img.size[1]/2.0))

            bg.paste(img, (xp-xadj, yp-yadj), img)
            img.close()

      # Save the background image with symbols.
...
class HorizontalColorbar(Colorbar):
    ...
    def draw_tick_labels(self, cbar):
        ...
        nlevels = len(levels)
        skip    = max(int(cbar.options.get('skip',1)),1)
        tkscale = cbar.options.get('tkscale', None)
        ...
        for n in range(nlevels):
            ...
            if n%skip == 0:
                ...
                label = float(levels[n])

                if tkscale == 'exp':
                    label = math.exp(label) + 0.01
                    label = '%0.4f'%(label,)
                else:
